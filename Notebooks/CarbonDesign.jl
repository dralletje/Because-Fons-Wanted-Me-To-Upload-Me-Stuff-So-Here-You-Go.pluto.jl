...
# ╔═╡ 47346df1-c40f-455f-9c5f-64bea7772efb
@enum CACHE_SOURCE FROM_FILE FROM_DOWNLOAD

# ╔═╡ afeeb3e5-a930-4f1d-99a4-45877ceb8d50
begin
	Base.@kwdef struct CacheResult
		content::String
		source::CACHE_SOURCE
		cache_mismatch::Union{Nothing,Bool}=nothing
	end
	function Base.show(io::IO, mime::MIME"text/plain", cache_result::CacheResult)
		show(io, mime, Text("From $(cache_result.source), cache_mismatch: $(cache_result.cache_mismatch)"))
	end
end

# ╔═╡ 34e07c9e-dfe9-4248-a05f-01e59096c5d1
macro cache(exprs...)
	exprs = map(exprs) do expr
		if Meta.isexpr(expr, :(=), 2)
			Expr(:kw, expr.args[1], expr.args[2])
		else
			expr
		end
	end
	quote
		# Check for PlutoHooks.@is_in_pluto() stuff eventually
		is_pluto = (
			isdefined(@__MODULE__, :PlutoRunner) &&
			parentmodule(@__MODULE__) == Main
		)
		cache($(map(esc, exprs)...), save=is_pluto)
	end
end
...
# ╔═╡ 1660538b-625c-455b-859b-d217692da8c3
md"### Doet het niet met overflow X"

# ╔═╡ 0157bc6d-d239-40fc-a37d-7b07908d4cf9
html"""<div style="height: 100px">"""

# ╔═╡ 2f881c44-207f-411a-890e-fdad0779b447
md"## Textarea example"
...
# ╔═╡ 69a52b81-4be9-4d7e-a336-52f4ffcdbc82
function cache(url; as, save=false)
	if save
		downloaded_result = sprint() do io
			Downloads.request(url, output=io)
		end

		cache_mismatch = if isfile(as)
			downloaded_result != read(as, String)
		else
			nothing
		end

		write(as, downloaded_result)

		CacheResult(
			content=downloaded_result,
			source=FROM_DOWNLOAD,
			cache_mismatch=cache_mismatch,
		)
	else
		if isfile(as)
			CacheResult(
				content=read(as, String),
				source=FROM_FILE,
			)
		else
			CacheResult(
...
# ╔═╡ c98f9fca-b920-4d9d-9c36-27d132f57ce1
custom_elements_json = @cache(
	"https://unpkg.com/carbon-web-components@1.18.0/custom-elements.json",
	as="carbon-web-components-custom-elements.json",
)
...
custom_elements_manifest = JSON3.read(
	custom_elements_json.content,
...
function render_carbon_component(;
	...
	@htl("""<dral-$tagname-wrapper>
		...
		<script>
			...
			let _value = element.value ?? element.checked
			Object.defineProperty(output_element, "value", {
				get: () => {
					return _value
				},
				set: (new_value) => {
					_value = value
					// Maybe check checked first?
...
@bind combined_form Form(@htl("""
	...
	$(CarbonDesign.Slider(name="some-text", [
		CarbonDesign.SliderInput()
	]))
"""))
